...
\begin{tikzpicture}[node distance=1cm, scale=0.8, every node/.style={scale=0.8}]
    % Distributed sources
    \node[server, minimum width=1.8cm, font=\scriptsize] (s1) at (-3, 2.5) {Server 1\\logs};
    \node[server, minimum width=1.8cm, font=\scriptsize] (s2) at (-3, 1.2) {Server 2\\logs};
    \node[client, minimum width=1.8cm, font=\scriptsize] (c1) at (-3, -0.1) {Client 1\\logs};
    \node[client, minimum width=1.8cm, font=\scriptsize] (c2) at (-3, -1.4) {Client 2\\logs};
    
    % Log Collector
    \node[component, minimum width=3cm, minimum height=2.2cm, fill=primary!20] (collector) at (0.5, 0.55) {
        \textbf{Log Collector}\\[4pt]
